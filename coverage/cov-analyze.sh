...
for remote in `ls $TEST_DIR/remote`; do
	...
	lcov -d $COV_DIR/ceph/src -c -o "$COV_DIR/${remote}_full.lcov" > /dev/null 2>&1
	lcov -r "$COV_DIR/${remote}_full.lcov" /usr/include\* -o "$COV_DIR/${remote}.lcov"
	lcov -a "$COV_DIR/${remote}.lcov" -a "$COV_DIR/${OUTPUT_BASENAME}.lcov" -o "$COV_DIR/${OUTPUT_BASENAME}.lcov" | tail -n 3
	rm "$COV_DIR/${remote}_full.lcov"
	rm $COV_DIR/ceph/src/*.gcda
done
